...
function resolveBackendDir(isDev: boolean, resourcesPath: string, appPath: string): string {
  // dev：仓库根目录下的 backend/；prod：打包后随 resources 分发的 backend/（M2 打包里程碑落地）
  return isDev ? path.join(appPath, 'backend') : path.join(resourcesPath, 'backend')
}

/** 启动 Python FastAPI sidecar，返回进程句柄与 baseUrl。
 * dev：backend/.venv（或系统 python）跑 uvicorn --reload；
